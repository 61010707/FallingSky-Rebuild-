...
Uint8 Collision::GetPixel(const sf::Uint8 * mask, const sf::Texture * tex, unsigned int x, unsigned int y)
{
...
sf::Uint8 * Collision::GetMask(const sf::Texture * tex)
{
	sf::Uint8* mask;
	std::map<const sf::Texture*, sf::Uint8*>::iterator pair = Bitmasks.find(tex);
	if (pair == Bitmasks.end()) { sf::Image img = tex->copyToImage(); mask = CreateMask(tex, img); }
	else mask = pair->second;

	return mask;
}

Uint8 * Collision::CreateMask(const sf::Texture * tex, const sf::Image & img)
{
	{
		...
		{
			for (unsigned int x = 0; x < tex->getSize().x; x++) mask[x + y * tex->getSize().x] = img.getPixel(x, y).a;
		}
...
bool Collision::PixelPerfectTest(const sf::Sprite & Object1, const sf::Sprite & Object2, sf::Uint8 AlphaLimit)
{
	...
	if (Object1.getGlobalBounds().intersects(Object2.getGlobalBounds(), Intersection)) {
		...
		sf::Uint8* mask1 = GetMask(Object1.getTexture());
		sf::Uint8* mask2 = GetMask(Object2.getTexture());
