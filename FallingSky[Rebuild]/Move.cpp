...
void Move::KeyboardMove(Sprite *sprite)
{
	...
	if (Keyboard::isKeyPressed(Keyboard::D)) { SpeedCharacter.x = speed; if (SpriteScale.x < 0.0f) { SpriteScale.x = -(SpriteScale.x); } }
	if (Keyboard::isKeyPressed(Keyboard::I)) { PlayerPos.y -= 1; }
	if (Keyboard::isKeyPressed(Keyboard::L)) { PlayerPos.x += 1; }
	if (Keyboard::isKeyPressed(Keyboard::K)) { PlayerPos.y += 1; }
	if (Keyboard::isKeyPressed(Keyboard::J)) { PlayerPos.x -= 1; }
